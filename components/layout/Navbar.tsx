...
export default function Navbar() {
  ...
  return (
    <header className="bg-white/90 backdrop-blur-md fixed w-full top-0 z-50 border-b border-gray-100 shadow-sm transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          {/* LOGO */}
          <div className="flex-shrink-0">
            <Link href="/" className="flex items-center gap-2 group">
              <span className="text-2xl font-black tracking-tighter text-[#064e3b] group-hover:opacity-80 transition-opacity">
                OCEAN<span className="text-[#f59e0b]">DU</span>NORD
              </span>
            </Link>
          </div>
          ...
          <nav className="hidden lg:flex space-x-6 items-center">
            ...
            <div className="relative group">
              <button className="text-gray-700 group-hover:text-[#064e3b] font-medium transition-colors flex items-center gap-1 py-4 text-sm">
                Océan du nord{" "}
              ...
              <div className="absolute left-0 mt-0 w-56 bg-white rounded-xl shadow-xl border border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all transform translate-y-2 group-hover:translate-y-0 overflow-hidden">
                <Link
                  ...
                  className="block px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 hover:text-[#064e3b]"
                >
                  Nos agences
                </Link>
                <Link
            ...
            <Link
              href="/actualites"
              className="text-gray-700 hover:text-[#064e3b] font-medium text-sm"
            >
              Actualités
            </Link>
            <Link
            ...
            <Link
              href="/oceane"
              className="flex items-center gap-2 text-[#f59e0b] font-bold bg-[#f59e0b]/10 px-3 py-1.5 rounded-full hover:bg-[#f59e0b] hover:text-white transition-colors text-xs"
            >
          ...
          <div className="hidden lg:flex items-center space-x-3">
            {user ? (
              ...
              <Link
                href="/login"
                className="text-[#064e3b] font-bold hover:bg-[#064e3b]/5 px-4 py-2 rounded-xl transition-colors text-sm"
              >
                Connexion
              </Link>
            )}
            <Link
              href="/booking"
              className="bg-[#064e3b] hover:bg-emerald-900 text-white px-6 py-2.5 rounded-full font-bold transition-all shadow-lg shadow-emerald-200 active:scale-95 text-sm"
            >
              Réserver
            </Link>
          </div>
          ...
          <div className="lg:hidden flex items-center gap-4">
            {!user && (
              <Link href="/login" className="p-2 text-[#064e3b]">
        ...
        <div className="p-6 space-y-6">
          ...
          {user && (
            <div className="bg-gray-50 p-4 rounded-2xl flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-[#064e3b] rounded-full flex items-center justify-center text-white font-bold uppercase">
                  {user.name?.charAt(0)}
                </div>
                <div>
          ...
          <div className="pt-4 space-y-3">
            <Link
              href="/oceane"
              onClick={toggleMenu}
              className="w-full flex items-center justify-center gap-2 p-4 bg-[#f59e0b]/10 text-[#f59e0b] font-black rounded-2xl transition-all"
            >
